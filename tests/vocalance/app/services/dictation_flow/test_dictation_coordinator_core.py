...
async def coordinator(mock_event_bus, mock_storage, app_config):
    ...
    with patch("vocalance.app.services.dictation_flow.dictation_coordinator.DictationTextInput"), patch(
        ...
        "vocalance.app.services.dictation_flow.dictation_coordinator.run_blocking",
...
@pytest.mark.asyncio
async def test_moonshine_engine_owned_by_coordinator(coordinator):
    """The coordinator owns its own MoonshineEngine; ``initialize`` must populate it."""
    assert coordinator.moonshine_engine is not None
...
async def test_initialization_with_all_services(mock_event_bus, mock_storage, app_config):
    ...
    with patch("vocalance.app.services.dictation_flow.dictation_coordinator.DictationTextInput") as mock_text, patch(
        "vocalance.app.services.dictation_flow.dictation_coordinator.LLMService"
    ) as mock_llm, patch(
        "vocalance.app.services.dictation_flow.dictation_coordinator.AgenticPromptService"
