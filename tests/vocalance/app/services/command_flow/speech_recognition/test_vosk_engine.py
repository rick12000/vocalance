...
def test_recognize_returns_text_on_success(vosk_engine_instance, mock_vosk_recognizer):
    audio_bytes = b"\x00\x01" * 100

    mock_vosk_recognizer.FinalResult.return_value = json.dumps({"text": "hello world"})

    result = vosk_engine_instance.recognize_sync(audio_bytes, sample_rate=16000)
...
def test_recognize_returns_empty_on_empty_audio(vosk_engine_instance):
    result = vosk_engine_instance.recognize_sync(b"", sample_rate=16000)

    assert result == ""


def test_recognize_returns_empty_when_no_text_recognized(vosk_engine_instance, mock_vosk_recognizer):
    audio_bytes = b"\x00\x01" * 100
...
def test_recognize_handles_exceptions_gracefully(vosk_engine_instance, mock_vosk_recognizer):
    audio_bytes = b"\x00\x01" * 100
    ...
    with pytest.raises(Exception, match="Recognition failed"):
        vosk_engine_instance.recognize_sync(audio_bytes, sample_rate=16000)
