...
class StreamlinedSoundService:
            ...
            if result:
                ...
                if not sound_label.startswith("esc50_"):
                    ...
                    recognition_event = CustomSoundRecognizedEvent(
                        label=sound_label, confidence=confidence, mapped_command=command or ""
                    )
    ...
    async def finish_training(self) -> bool:
        ...
        try:
            ...
            success = await self.recognizer.train_sound(self._current_training_label, self._training_samples)

            if success:
                logger.info(
                    f"Training completed for '{self._current_training_label}' " f"with {len(self._training_samples)} samples"
                )
                await self.event_bus.publish(SoundTrainingCompleteEvent(sound_name=self._current_training_label, success=True))
            else:
                logger.error(f"Training failed for '{self._current_training_label}'")
                await self.event_bus.publish(
                    SoundTrainingFailedEvent(sound_name=self._current_training_label, reason="Training failed")
                )
            ...
            await self.event_bus.publish(SoundTrainingFailedEvent(sound_name=self._current_training_label, reason=str(e)))
            self._reset_training_state()
            return False
    ...
    async def _handle_training_request(self, event_data: SoundTrainingRequestEvent):
        ...
        try:
            ...
            await self.event_bus.publish(
                SoundTrainingInitiatedEvent(sound_name=event_data.sound_label, total_samples=event_data.num_samples)
            )
    ...
    async def _handle_map_sound_command(self, event_data: MapSoundToCommandPhraseCommand):
        ...
        try:
            ...
            await self.event_bus.publish(
                SoundToCommandMappingUpdatedEvent(
                    sound_label=event_data.sound_label, command_phrase=event_data.command_phrase, success=True
                )
            )
            ...
            await self.event_bus.publish(
                SoundToCommandMappingUpdatedEvent(
                    sound_label=event_data.sound_label, command_phrase=event_data.command_phrase, success=False
                )
            )
