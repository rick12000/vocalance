...
class SmartTimeoutManager:
    ...
    def __init__(self, app_config: GlobalAppConfig, command_action_map: Optional[Dict[str, tuple]] = None):
        ...
        logger.info(
            f"SmartTimeoutManager initialized: {len(self._all_commands)} commands, "
    ...
    def _build_ambiguity_analysis(self, commands: Set[str]) -> None:
        ...
        for command in commands_list:
            ...
            is_prefix_of_others = any(other_cmd.startswith(command + " ") for other_cmd in commands_list if other_cmd != command)
