...
HOMOPHONES = {
    'won': 'one', 'to': 'two', 'too': 'two', 'free': 'three', 
    'for': 'four', 'fore': 'four', 'ate': 'eight'
}
...
def detect_digit_sequence(text):
    ...
    if (len(words) > 1 and 
        all(word in SINGLE_DIGIT_WORDS for word in words) and
        not any(word in SCALE_WORDS for word in words)):
...
def parse_number(text, min_value=1, max_value=5000):
    ...
    if text is None or text == "":
        logger.debug(f"[NumberParser] Text is None or empty")
        return None
