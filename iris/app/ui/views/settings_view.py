...
class SettingsView(ctk.CTkFrame):
    ...
    def _build_tab_ui(self):
        ...
        markov_frame.grid(row=2, column=0, sticky="ew", 
                         padx=(ui_theme.theme.two_box_layout.outer_padding_left, ui_theme.theme.two_box_layout.outer_padding_right), 
        ...
        markov_header.grid(row=0, column=0, columnspan=3, 
                          padx=ui_theme.theme.spacing.medium, 
                          pady=(ui_theme.theme.spacing.medium, ui_theme.theme.spacing.small), 
                          sticky="w")
        ...
        self.markov_confidence_entry = ThemedEntry(
            markov_frame, 
            textvariable=self.markov_confidence_var,
            width=ui_theme.theme.dimensions.entry_width_small
        )
        self.markov_confidence_entry.grid(row=1, column=1, padx=ui_theme.theme.spacing.medium, pady=ui_theme.theme.spacing.small, sticky="w")
        
        # Markov confidence description
        markov_desc = ThemedLabel(markov_frame, text="(0.0-1.0, threshold for using Markov vs speech model)", color=ui_theme.theme.text_colors.medium)
        ...
        markov_save_button = PrimaryButton(
            markov_buttons_frame,
            text="Save Markov Settings",
            command=self._save_markov_settings
        )
        ...
        markov_reset_button = DangerButton(
            markov_buttons_frame,
            text="Reset to Defaults",
            command=self._reset_markov_to_defaults
        )
        ...
        sound_frame.grid(row=3, column=0, sticky="ew", 
                        padx=(ui_theme.theme.two_box_layout.outer_padding_left, ui_theme.theme.two_box_layout.outer_padding_right), 
                        pady=(ui_theme.theme.spacing.small, ui_theme.theme.spacing.medium))
        
        # Sound Settings Header
        sound_header = BoxTitle(sound_frame, text="Sound Recognizer Settings")
        sound_header.grid(row=0, column=0, columnspan=3, 
                         padx=ui_theme.theme.spacing.medium, 
                         pady=(ui_theme.theme.spacing.medium, ui_theme.theme.spacing.small), 
                         sticky="w")
        ...
        self.sound_confidence_entry = ThemedEntry(
            sound_frame, 
            textvariable=self.sound_confidence_var,
            width=ui_theme.theme.dimensions.entry_width_small
        )
        self.sound_confidence_entry.grid(row=1, column=1, padx=ui_theme.theme.spacing.medium, pady=ui_theme.theme.spacing.small, sticky="w")
        
        # Sound confidence description
        sound_conf_desc = ThemedLabel(sound_frame, text="(0.0-1.0, minimum similarity for recognition)", color=ui_theme.theme.text_colors.medium)
        ...
        self.sound_vote_entry = ThemedEntry(
            sound_frame, 
            textvariable=self.sound_vote_var,
            width=ui_theme.theme.dimensions.entry_width_small
        )
        self.sound_vote_entry.grid(row=2, column=1, padx=ui_theme.theme.spacing.medium, pady=ui_theme.theme.spacing.small, sticky="w")
        
        # Sound vote description
        sound_vote_desc = ThemedLabel(sound_frame, text="(0.0-1.0, minimum vote alignment percentage)", color=ui_theme.theme.text_colors.medium)
        ...
        sound_save_button = PrimaryButton(
            sound_buttons_frame,
        ...
        sound_reset_button = DangerButton(
            sound_buttons_frame,
            text="Reset to Defaults",
            command=self._reset_sound_to_defaults
        )
        ...
        try:
            ...
            if settings:
                ...
                for var in [self.llm_context_length_var, self.llm_max_tokens_var, self.grid_default_cells_var, 
                           self.markov_confidence_var, self.sound_confidence_var, self.sound_vote_var]:
    ...
    def _save_markov_settings(self):
        ...
        self.controller.save_markov_settings(
            self.markov_confidence_var.get()
        )
    
    def _reset_markov_to_defaults(self):
        """Reset Markov settings to defaults through controller"""
        result = messagebox.askyesno(
            "Reset Markov Settings", 
            "Are you sure you want to reset Markov Chain settings to defaults?", 
            parent=self.root_window
        )
    ...
    def _save_sound_settings(self):
        ...
        self.controller.save_sound_settings(
            self.sound_confidence_var.get(),
            self.sound_vote_var.get()
        )
