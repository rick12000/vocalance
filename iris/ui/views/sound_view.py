...
from iris.ui.views.components.base_view import BaseView
from iris.ui.views.components.form_builder import FormBuilder
from iris.ui.views.components.view_config import view_config
from iris.ui.views.components.themed_components import (
...
class SoundView(BaseView):
    ...
    def _setup_training_form(self) -> None:
        ...
        self.samples_label, self.samples_entry = FormBuilder.create_labeled_entry(
            container, "Number of samples:", view_config.form_defaults.placeholder_samples, 
            default_value=view_config.form_defaults.placeholder_samples, row=2
        )
        
        # Add button - store reference to the button frame
        self.record_button_frame = ctk.CTkFrame(container, fg_color="transparent")
        self.record_button_frame.grid(row=4, column=0, sticky="ew", 
        ...
        self.record_button = PrimaryButton(
            ...
            text=view_config.theme.button_text.record,
        ...
        # Progress bar
        self.training_progress_bar = ctk.CTkProgressBar(self.training_status_frame, fg_color=view_config.theme.shape_colors.lightest, progress_color=view_config.theme.text_colors.medium)
    ...
    def _toggle_training_interface(self, training_active: bool) -> None:
        ...
        input_widgets = [self.name_label, self.sound_name_entry, 
                        self.samples_label, self.samples_entry, self.record_button_frame]
        ...
        if training_active:
            self.training_status_frame.grid(row=4, column=0, sticky="nsew", 
                                          padx=view_config.theme.spacing.medium, 
    ...
    def _create_sound_item(self, sound_name: str, row_index: int) -> None:
        ...
        sound_frame.grid(row=row_index, column=0, sticky="ew", 
        ...
        # Status
        status_color = (view_config.theme.text_colors.medium if mapping_status == "Unmapped" 
        ...
        # Buttons
        PrimaryButton(sound_frame, text=view_config.theme.button_text.map, compact=True,
                      command=lambda s=sound_name: self._map_sound_to_command(s)).grid(
            row=0, column=2, padx=view_config.theme.spacing.tiny
        )
        
        DangerButton(sound_frame, text=view_config.theme.button_text.delete, compact=True,
                     command=lambda s=sound_name: self._delete_sound(s)).grid(
            row=0, column=3, padx=view_config.theme.spacing.tiny
        )
    ...
    def _map_sound_to_command(self, sound_name: str) -> None:
        ...
        label_font = (theme.font_family.primary, theme.font_sizes.medium)
        ...
        type_dropdown.grid(row=1, column=0, sticky="ew", 
                          pady=(0, theme.spacing.small), 
                          padx=theme.spacing.medium)
        ...
        value_dropdown.grid(row=3, column=0, sticky="ew", 
                           pady=(0, theme.spacing.small), 
                           padx=theme.spacing.medium)

        # Store references for the callback
        self._temp_dialog_refs = {
            'type_var': type_var,
            'value_var': value_var,
