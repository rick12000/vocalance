...
class GridViewConfig(BaseModel):
    ...
    rect_fill_alpha: float = Field(default=0.0, description="Alpha transparency for rectangle fill - 0.0 for transparent cells")
    rect_outline_alpha: float = Field(default=0.6, description="Alpha transparency for rectangle outline (0.0-1.0)")
    text_alpha: float = Field(default=0.6, description="Alpha transparency for text labels (0.0-1.0)")
...
class DictationPopupConfig(BaseModel):
    ...
    def simple_window_size(self) -> tuple:
        ...
        return (ui_theme.theme.dimensions.dictation_simple_width, 
                ui_theme.theme.dimensions.dictation_simple_height)
    
    @property
    def smart_window_size(self) -> tuple:
        """Get smart window dimensions from ui_theme"""
        return (ui_theme.theme.dimensions.dictation_smart_width,
                ui_theme.theme.dimensions.dictation_smart_height)
