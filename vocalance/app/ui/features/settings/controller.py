...
class QtSettingsController(QtBaseController):
    ...
    def __init__(
        ...
        self._llm_bundle_status: Dict[str, bool] = {}
        self._pending_setting_futures: Dict[str, asyncio.Future[Tuple[bool, str]]] = {}
        self._active_llm_download_rid: Optional[str] = None

        self._subscribe_to_events()
        self.logger.debug("QtSettingsController initialized")

    def _subscribe_to_events(self) -> None:
        self.event_bus.subscribe(SettingsChangedEvent, self._handle_settings_changed)
        self.event_bus.subscribe(RuntimeConfigResponseEvent, self._on_runtime_config_response)
        self.event_bus.subscribe(LlmUiNotificationEvent, self._on_llm_ui_notification)
    ...
    def _on_runtime_config_response(self, event: RuntimeConfigResponseEvent) -> None:
        if event.op == "effective_snapshot":
            self._cached_settings = event.all_settings
            self.settings_loaded.emit(self._cached_settings)
            asyncio.create_task(self.event_bus.publish(LlmUiRequestEvent(op="refresh_bundle_status")))
        elif event.op == "update_result":
            fut = self._pending_setting_futures.pop(event.correlation_id, None)
    ...
    def _on_llm_ui_notification(self, event: LlmUiNotificationEvent) -> None:
        if event.kind == "bundle_status":
            self._llm_bundle_status = dict(event.status)
        elif event.kind == "download_progress":
            if self._active_llm_download_rid and event.request_id == self._active_llm_download_rid:
            ...
            if self._active_llm_download_rid and event.request_id == self._active_llm_download_rid:
                ...
                self._active_llm_download_rid = None
            asyncio.create_task(self.event_bus.publish(LlmUiRequestEvent(op="refresh_bundle_status")))
    ...
    async def update_setting_async(self, key: str, value: Any) -> Tuple[bool, str]:
        ...
        self._pending_setting_futures[cid] = fut
    ...
    def update_setting(self, key: str, value: Any) -> None:
        asyncio.create_task(self._update_setting_task(key, value))

    async def _update_setting_task(self, key: str, value: Any) -> None:
    ...
    def llm_bundle_on_disk(self, model_id: str) -> bool:
        if not get_whitelisted_llm_model(model_id):
            return False
        return bool(self._llm_bundle_status.get(model_id))

    def schedule_llm_cancellable_download(self, model_id: str) -> str:
        rid = str(uuid.uuid4())
        self._active_llm_download_rid = rid
    ...
    def cancel_llm_download(self) -> None:
        rid = self._active_llm_download_rid or ""
    ...
    async def _reset_to_defaults_async(self) -> None:
        await self.event_bus.publish(RuntimeConfigRequestEvent(op="reset_defaults"))
    ...
    def cleanup(self) -> None:
        try:
            ...
            self.event_bus.unsubscribe(RuntimeConfigResponseEvent, self._on_runtime_config_response)
            self.event_bus.unsubscribe(LlmUiNotificationEvent, self._on_llm_ui_notification)
        except Exception as e:
