    ...
    def _draw_marks(self):
        ...
        for label, (x, y) in self.marks.items():
            ...
            self.canvas.create_oval(
                ...
                x + radius,
                y + radius,
                fill=view_config.marks.themed_mark_fill_color,
                outline=view_config.marks.themed_mark_outline_color,
            )
            # Draw the label near the mark
            self.canvas.create_text(x + 10, y - 10, text=label, fill="white", anchor="w", font=mark_font)
